...
import requests, socket, signal, urllib.request, json
    ...
    def run(self):
        ...
        for site in self.sites:
            if self.check(site) == True:
                ...
                try:
                    with Pool(self.thread_count, self.init_worker) as finder_pool:
                        try: finder_pool.map(self.find, self.urls)
                        except KeyboardInterrupt:
                            finder_pool.terminate()
                            finder_pool.join()
                            exit("\nUser exits")
                        else:
                            finder_pool.close()
                    ...
                    exit("\n\x1b[0;1mError: Cant create pool, exiting\x1b[0m")
